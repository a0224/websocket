...
import com.bao8.websocket.request.Request;
import com.bao8.websocket.response.ErrorResponse;
import com.bao8.websocket.response.Response;
import com.bao8.websocket.response.ResponseFactory;
...
public class WebSocketWrapper {
    ...
    private static final String TAG = "WebSocket-WebSocketWrapper";
    ...
    void disConnect() {
        ...
        if (connectStatus == 2) {
            ...
            LogUtil.i(TAG, "WebSocket disconnected  url:" + mSetting.getConnectUrl());
        }
    }
    ...
    private void onWSCallbackOpen(ServerHandshake handshakeData) {
        ...
        connectStatus = 2;
        LogUtil.i(TAG, "WebSocket connect success  url:" + mSetting.getConnectUrl());
        if (needClose) {
            disConnect();
        } else {
            if (mSocketListener != null) {
    ...
    private void onWSCallbackClose(int code, String reason, boolean remote) {
        connectStatus = 0;
        LogUtil.e(TAG, String.format("WebSocket closed!  code=%s,reason:%s,remote:%s", code, reason, remote));
        if (mSocketListener != null) {
            mSocketListener.onDisconnect();
        }
        checkDestroy();
